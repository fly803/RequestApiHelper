...
public interface RequestApiInterface {
  ...
  @GET("v2/book/search") Call<BookSearchResponse> callTypeGet(@Query("q") String name,
   ...
   */
  @FormUrlEncoded @Headers({ "baseurl:gank" }) @POST("api/add2gank")
  //    @POST(UrlConstants.postDataUrl)
  Call<Object> callTypePost(@Field("url") String url, @Field("desc") String desc,
      @Field("who") String who, @Field("type") String type, @Field("debug") Boolean debug);

  @GET("service/getIpInfo.php") Observable<BaseResponse<IpResult>> rxGet(@Query("ip") String ip);
   ...
   * @version 1.0
   */
  @GET("test/cg") Observable<BaseResponse<MyResponse.DataBean>> psalms(@Query("param") int type);
